...
void UI_Delete(u8 Del_Operate, u8 Del_Layer) {
  ...
  p += sizeof(framehead);
  ui.len += sizeof(framehead);

  framepoint = (unsigned char *)&datahead;
  memcpy(p, framepoint, sizeof(datahead));
  p += sizeof(datahead);
  ui.len += sizeof(datahead);
...
void Float_Draw(Float_Data *image, char imagename[3], u32 Graph_Operate, u32 Graph_Layer, u32 Graph_Color,
...
** Output: CRC checksum
*/
uint16_t Get_CRC16_Check_Sum_UI(u8 *pchMessage, uint32_t dwLength, uint16_t wCRC) {
  u8 chData;
  if (pchMessage == NULL) {
    return 0xFFFF;
  }
